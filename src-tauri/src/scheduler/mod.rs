...
pub fn get_engine() -> Option<SharedEngine> { ENGINE.get().cloned() }

// ── 用户空闲检测（用 64 位 API 防溢出） ──
...
fn get_idle_seconds() -> u64 {
    #[repr(C)] struct LASTINPUTINFO { cbSize: u32, dwTime: u32 }
    extern "system" { fn GetLastInputInfo(plii: *mut LASTINPUTINFO) -> i32; fn GetTickCount64() -> u64; }
    unsafe {
...
pub fn init(app: &AppHandle) -> tauri::Result<()> {
    ...
    tauri::async_runtime::spawn(async move { tick_loop(app_clone, engine_clone).await; });
...
async fn handle_reminder_triggered(app: &AppHandle, reminder_type: &str) {
    ...
    let _ = app.notification().builder().title(&payload.title).body(&payload.body).show();
...
pub async fn set_interval_minutes(ty: &str, minutes: u64) {
    if let Some(eng) = get_engine() { eng.lock().await.set_interval_minutes(ty, minutes); }
}
pub async fn get_snapshot() -> Option<EngineSnapshot> {
    if let Some(eng) = get_engine() { Some(eng.lock().await.snapshot()) } else { None }
}
